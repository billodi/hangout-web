...
export async function POST(req: Request) {
  ...
  if (file.size <= 0 || file.size > 5_000_000) return Response.json({ error: "File too large" }, { status: 400 });
  if (!["image/png", "image/jpeg", "image/webp"].includes(file.type)) {
    return Response.json({ error: "Unsupported file type. Use PNG, JPG, or WEBP." }, { status: 400 });
  }

  try {
    const url = await uploadImageToCloudinary({ userId: user.id, file, kind: "avatars" });
